...
import database, invoice_generator
import sv_ttk
...
class BuyCarPage:
    ...
    def __init__(self):
        ...
        self.space = ttk.Label(self.frame2, text="Available cars", width=100, background="white", font=('Bahnschrift SemiBold Condensed', 30, 'normal'), anchor= CENTER)
        ...
        for i in d:
            ...
            self.car_type = ttk.Label(self.frame2, text=f"Car type : {i[1]}                  Brand : {i[2]}                       Model : {i[3]}", width=100, foreground='#57A1F8', background="light grey", font=('Bahnschrift SemiBold Condensed', 17, 'normal'))
            self.car_type.pack(ipady=10)

            self.car_brand = ttk.Label(self.frame2, text=f"Engine : {i[4]}                  Mileage : {i[5]}                KM Driven : {i[6]}", width=100, foreground='#57A1F8', background="light grey", font=('Bahnschrift SemiBold Condensed', 17, 'normal'))
            self.car_brand.pack(ipady=10)

            self.car_variant = ttk.Label(self.frame2, text=f"Ownership : {i[8]}                Registeration Year : {i[7]}", width=100, foreground='#57A1F8', background="light grey", font=('Bahnschrift SemiBold Condensed', 17, 'normal'))
            self.car_variant.pack(ipady=10)

            self.ownership = ttk.Label(self.frame2, text=f"Price : {i[9]}", width=100, foreground='#57A1F8', background="light grey", font=('Bahnschrift SemiBold Condensed', 17, 'normal'))
            self.ownership.pack(ipady=10, anchor= N)

            self.c1 = ttk.Button(self.frame2, text=' BUY ', command= self.buy)
            self.c1.pack(ipadx=10, ipady=5, anchor= N)
    ...
    def buy(self):
        self.root.destroy()
        buy = invoice_generator.InvoiceGenerator()
        buy.widgets()



        # Pack the frames into the canvas
        self.canvas.create_window((0, 0), window= self.frame2, anchor="sw", height=2000, width=1000)
        self.canvas.pack(side="bottom", fill="both", expand=True)
